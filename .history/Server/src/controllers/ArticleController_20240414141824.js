...
exports.getTotalArticlesByAcademicYear = async (req, res) => {
    try {
        const academicYears = await Academic.find(); // Lấy danh sách tất cả các khoa
        const totalArticlesByAcademicYear = [];
        // Loop qua từng năm
        for(const academic of academicYears) {
            const articles = await Academic
        }
    } catch (error) {
        console.error(
            "Error getting total number of articles by academic year:",
