...
//             const marketingC = await User.findById(
...
//             // Nếu không tồn tại, tạo thư mục temp
...
//             zlib: { level: 9 }, // set level of compression
//         });
...
//                 fileName
//             );
//             console.log("Adding file to zip:", filePath); // Ghi log để theo dõi quá trình thêm file vào zip
//             archive.append(fs.createReadStream(filePath), {
//                 name: article._id.toString(),
//             }); // Sử dụng id của article làm tên file trong zip
//         });
...
//     } catch (error) {
//         console.error(
//             "Error getting total number of articles by faculty:",
//             error
//         );
//         res.status(500).json({ message: "Internal server error" });
//     }
...
//     } catch (error) {
//         console.error(
//             "Error getting total number of articles by status:",
//             error
//         );
//         res.status(500).json({ message: "Internal server error" });
//     }
...
//         let withComments = 0;
//         let withoutComments = 0;
//         let withoutCommentsAfter14Days = 0;
...
//             // Kiểm tra xem bài viết đã được tạo trước 14 ngày hay không
//             if (moment(article.submitDate).isBefore(fourteenDaysAgo)) {
//                 const commentsAfter14Days = comments.filter((comment) =>
//                     moment(comment.submitDate).isAfter(fourteenDaysAgo)
//                 );

//                 // Kiểm tra xem có bình luận nào được thêm vào sau 14 ngày đó không
//                 if (commentsAfter14Days.length === 0) {
//                     withoutCommentsAfter14Days++;
//                 }
//             }

//             // Kiểm tra xem bài viết có bình luận không
...
exports.getTotalByAcademicYear = async (req, res) => {
    try {
        // Lấy tất cả các bài viết từ cơ sở dữ liệu
        const articles = await Article.find();
...
exports.getTotalContributor = async (req, res) => {
    try {
        const faculties = await Faculty.find(); // Lấy danh sách tất cả các khoa
        const totalUniqueUsersWithArticlesByFaculty = [];
        ...
        for (const faculty of faculties) {
            ...
            // Tìm số lượng người dùng duy nhất có ít nhất một bài viết
            const uniqueUsersWithArticlesCount = await Article.aggregate([
                { $match: { userId: { $in: userIds } } }, // Lọc các bài viết của người dùng trong khoa
                { $group: { _id: "$userId" } }, // Nhóm các bài viết theo userId để loại bỏ trùng lặp
                { $count: "count" }, // Đếm số lượng người dùng duy nhất có bài viết
            ]);

            const count =
                uniqueUsersWithArticlesCount.length > 0
                    ? uniqueUsersWithArticlesCount[0].count
                    : 0;

            // Lưu vào đối tượng kết quả với tên khoa và số lượng người dùng duy nhất có bài viết
            totalUniqueUsersWithArticlesByFaculty.push({
                facultyName: faculty.name,
                uniqueUsersWithArticlesCount: count,
            });
        }

        res.status(200).json({ data: totalUniqueUsersWithArticlesByFaculty });
    } catch (error) {
        console.error(
            "Error getting total number of unique users with articles by faculty:",
            error
        );
        res.status(500).json({ message: "Internal server error" });
    }
