...
exports.getTotalContributor = async (req, res) => {
    try {
        const faculties = await Faculty.find(); // Lấy danh sách tất cả các khoa
        const totalUsersWithArticlesByFaculty = [];
        ...
        for (const faculty of faculties) {
            ...
            const countedUsers = new Set(); // Tạo một Set để lưu trữ các ID của người dùng đã được đếm

            // Lặp qua tất cả bài viết của khoa hiện tại
            const articles = await Article.find({ userId: { $in: userIds } });
            for (const article of articles) {
                // Kiểm tra xem ID của người dùng đã được đếm chưa
                if (!countedUsers.has(article.userId)) {
                    // Nếu chưa, tăng biến đếm và thêm ID của người dùng vào Set
                    countedUsers.add(article.userId);
                }
            }

            // Số lượng người dùng đã được đếm chính là kích thước của Set
            const usersWithArticlesCount = countedUsers.size;

            // Lưu vào đối tượng kết quả với tên khoa và số lượng người dùng có bài viết
            totalUsersWithArticlesByFaculty.push({
                facultyName: faculty.name,
                usersWithArticlesCount,
            });
        }

        res.status(200).json({ data: totalUsersWithArticlesByFaculty });
    } catch (error) {
        console.error(
