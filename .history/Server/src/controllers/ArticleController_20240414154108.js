...
exports.getTotalContributor = async (req, res) => {
    try {
        ...
        for (const faculty of faculties) {
            ...
            // Tìm số lượng người dùng có ít nhất một bài viết
            const usersWithArticlesCount = await Article.countDocuments({ userId: { $in: userIds } });

            // Lưu vào đối tượng kết quả với tên khoa và số lượng người dùng có bài viết
            totalUsersWithArticlesByFaculty.push({ facultyName: faculty.name, usersWithArticlesCount });
        }

        res.status(200).json({ data: totalUsersWithArticlesByFaculty });
    } catch (error) {
        console.error("Error getting total number of users with articles by faculty:", error);
        res.status(500).json({ message: "Internal server error" });
    }
}
