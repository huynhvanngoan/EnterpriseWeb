...
const ArticleManagerStudent = () => {
  ...
  useEffect(() => {
      // Kiểm tra và cập nhật trạng thái vô hiệu hóa nút khi giá trị finalClosureDate thay đổi
      const checkClosureDate = () => {
          // Lặp qua danh sách academics để kiểm tra finalClosureDate
          academics.forEach((academic) => {
              // Chuyển finalClosureDate thành đối tượng Date
              const closureDate = new Date(academic.finalClosureDate);
              console.log("check", closureDate);
              // Kiểm tra nếu finalClosureDate trước thời điểm hiện tại
              if (closureDate < Date.now()) {
                  // Nếu trước, vô hiệu hóa nút chỉnh sửa và xóa
                  setIsButtonDisabled(true);
              } else {
                  // Nếu sau, không vô hiệu hóa nút
                  setIsButtonDisabled(false);
              }
          });
      };

      // Gọi hàm kiểm tra khi component được render và mỗi khi giá trị của academics thay đổi
      checkClosureDate();
  }, [academics]);
  ...
  const isClosureDateExpired = (closureDate) => {
      const today = new Date();
      return new Date(closureDate) < today;
  };
  const columns = [
      {
          title: "ID",
                  ...
                  <Row>
                      <div style={{ marginLeft: 10 }}>
                          ...
                          <Button
                              ...
                              disabled={
                                  isButtonDisabled ||
                                  isClosureDateExpired(record.finalClosureDate)
                              }
                          >
                              Edit
                          </Button>
